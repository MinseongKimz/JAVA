...
○ 객체 직렬화(Object Serialization)는
   ...
   멤버 변수의 갑슬 보존한다거나
   다른 네트워크에 있는 호스트에 값을 보낼 경우에 사용하게 된다.
...
○ Serializable 인터페이스
   ...
   객체 직렬화 하기 위해 먼저 객체 직렬화가 가능하도록
   java.io.Serializable 인터페이스를 구현해 주어야 한다.
   이 인터페이스는 객체 직려화가 제공되어야 한다는 사실을
   ...
   ② 
   Serilizable 인터페이스를 구현한 후
   ObjectInputStream 클래스와 ObjectOutputStream을 이용하여
   객체 단위로 입출력을 수행하게 된다.
...
○ Object Stream
   ...
   ObjectInputStream 클래스는 ObjectOutputStream 클레스에 의해
   파일에 저장되어 있는 객체나 네트워크를 통해 전달된 객체의
   직렬화를 해제하는 기능을 제공한다.
   단, java.io.Serializable와
...
public class Test179
{
	public static void main(String[] args) throws Exception
	{
		...
		//--==>> false
		//-- 『test.ser』 파일이 만들어질 디렉토리 경로가 구성 X
			
		//테스트 ②
		//C:\JavaStudy』 경로에 『data』 디렉토리 생성한 후 확인
		//System.out.println(f0.getParentFile().exists());
		//--==>> true
		//-- 『test.ser』파일이 만들어질 디렉토리 경로가 구성되있다.
 
		if (!f0.getParentFile().exists()) // 경로가 없다면
			f0.getParentFile().mkdirs(); // 만들어라, makedirectorys 
