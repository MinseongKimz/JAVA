...
○	중첩 클래스
	...
	만들수 있는데 이러한 클래스를 중첩 클래스라 한다.
	이는 특정 클래스를 자신의 클래스 내부적인 용도로만
	사용 할 목적으로 쓰이는데
	특정 클래스를 마치 자신의 멤버변수나 메소드처럼 사용할 수 있게 한다.

○	중첩 클래스의 종류(4가지)
	
	1.	static 중첩 클래스(중첩 내부 클래스)
		클래스 내부에...『public static class 클래스 명』
	...
	2.	내부 클래스 (inner class)
		...
		- static 중첩 클래스와 마찬가지로 프로그램 구조를 보다 더 간단하고 알아보기
		  쉽도럭 할 수 있다.
	...
	3.	지역 클래스 (로컬 클래스, local class)
		메소드 내부에 『public class 클래스명』또는 『public static class 클래스명』

		- 클래스의 메소드 안에서 클래스를정의하는 것으로
...
class Test
{
	...
	public static class StaticNested	// static
	{
		...
		void write()
		{
			...
			//System.out.println("ob2.b : "+ b); // 이 클래스는 static 클래스이므로 b보다 탄생시점 먼저
...
public class Test124
{
	public static void main(String[] args)
	{
		...
		//StaticNested ob2 = new StaticNested(); // static 클래스.. 
		//--==>> 에러 발생

		//※ 중첩 내부 클래스는 외부에서 단독으로 객체를 생성한다.
		//   단, 위와 같은 방법으로 객체를 생성해서는 안되고
		//	 클래스 변수 접근이나 클레스 메소드를 호출하는 것과 같은 방식을 통해
